...
function html_showportfolios( $rows,  $pageNav,$sort,$cat_row){
	global $wpdb;
        $wp_nonce = wp_create_nonce('huge_it_portfolio_check');
	?>
...
<div class="wrap">
	<?php $path_site2 = plugins_url("../images", __FILE__); ?>
		...
		/***fvpps***/
		#Lfree_portfolio #posts-portfolio-heading .buttons.portfolio-image .wp-medias-buttons-icon {
		...
		.free_portfolio_versions_banner .get_fulls_after_version:hover {
			background:#CCCCCC;
			color:#bf1e6e;
	...
	<div id="poststuff">
		<div id="portfolios-list-page">
			...
			<h2><?php echo __( 'Huge-IT Portfolios', 'portfolio-gallery' );?>
				<a onclick="window.location.href='admin.php?page=portfolios_huge_it_portfolio&task=add_cat&huge_it_portfolio_nonce=<?php echo $wp_nonce;?>'" class="add-new-h2" ><?php echo __( 'Add New Portfolio', 'portfolio-gallery' );?></a>
			</h2>
			<?php
			$serch_value='';
				 ...
				 $trcount=1;
				  for($i=0; $i<count($rows);$i++){
					...
					<tr <?php if($trcount%2==0){ echo 'class="has-background"';}?>>
						...
						<td><a  href="admin.php?page=portfolios_huge_it_portfolio&task=edit_cat&id=<?php echo $rows[$i]->id?>&huge_it_portfolio_nonce=<?php echo $wp_nonce;?>"><?php echo esc_html(stripslashes($rows[$i]->name)); ?></a></td>
...

{
 ...
 $wp_nonce = wp_create_nonce('huge_it_portfolio_check');
	...
	header('Location: admin.php?page=portfolios_huge_it_portfolio&id='.$strid.'&task=apply&huge_it_portfolio_nonce='.$wp_nonce);
	}
	}
...
function submitbuttonF(pressbutton) 
{
	...
	filterInputs();		
	document.getElementById("adminForm").action=document.getElementById("adminForm").action+"&task="+pressbutton+"&huge_it_portfolio_nonce=<?php echo $wp_nonce;?>";
	...
	<div id="portfolio-header">
		<ul id="portfolios-list">
			...
			foreach($rowsld as $rowsldires){
				...
				?>
					<li>
						<a href="#" onclick="window.location.href='admin.php?page=portfolios_huge_it_portfolio&task=edit_cat&id=<?php echo $rowsldires->id; ?>&huge_it_portfolio_nonce=<?php echo $wp_nonce;?>'" ><?php echo $rowsldires->name; ?></a>
					</li>
				<?php
				}
				else{ ?>
		...
		?>
			<li class="add-new">
				<a onclick="window.location.href='admin.php?page=portfolios_huge_it_portfolio&amp;task=add_cat&huge_it_portfolio_nonce=<?php echo $wp_nonce;?>'">+</a>
			</li>
		</ul>
		</div>
							...
							<div class="image-options">
								<div class="options-container">
									...
									<div class="link-block">
										...
										<input class="text_area url-input" type="text" id="sl_url<?php echo $rowimages->id; ?>" name="sl_url<?php echo $rowimages->id; ?>"  value="<?php echo esc_html(stripslashes($rowimages->sl_url)); ?>" >
								...
								<div class="remove-image-container">
									<a class="button remove-image" href="admin.php?page=portfolios_huge_it_portfolio&id=<?php echo esc_html(stripslashes($row->id)); ?>&task=apply&removeslide=<?php echo $rowimages->id; ?>&huge_it_portfolio_nonce=<?php echo $wp_nonce; ?>"><?php echo __( 'Remove Project', 'portfolio-gallery' );?></a>
								</div>
							</div>                 
							<div class="clear"></div>
						</li>
...
//                                        });
					...
					<ul id="portfolio-unique-options-list">
						...
						<div id="major-publishing-actions">
							<div id="publishing-action">
								<input type="button" onclick="submitbuttonF('apply')" value="Save Portfolio" id="save-buttom" class="button button-primary button-large">
							</div>
							<div class="clear"></div>
	...
	<?php wp_nonce_field('huge_it_portfolio','huge_it_portfolio_check'); ?>
</form>
</div>
