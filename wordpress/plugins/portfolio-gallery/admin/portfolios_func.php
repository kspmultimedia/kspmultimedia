...
function editportfolio($id)
  {
	  global $wpdb;
            ...
            if(!wp_verify_nonce($wp_nonce, 'huge_it_portfolio_check')) {
...
( 'New portfolio', '375', '600', 'on', 'cubeH', '4000', '1000', 'off', '1', '300', 'My First Category,My Second Category,My Third Category,', 'off', 'off')";
   ...
   $query="SELECT * FROM ".$wpdb->prefix."huge_itportfolio_portfolios order by id ASC";
	...
	foreach($rowsldcc as $key=>$rowsldccs){
		if($last_key == $key){
                        $wp_nonce = $_GET['huge_it_portfolio_nonce'];
			header('Location: admin.php?page=portfolios_huge_it_portfolio&id='.$rowsldccs->id.'&task=apply&huge_it_portfolio_nonce='.$wp_nonce);
		}
	}	
}
...
{	
	...
	$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itportfolio_portfolios SET  ordering = '1'  WHERE id = %d ", $id));
        $wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itportfolio_portfolios SET  categories = '%s'  WHERE id = %d ", $_POST["allCategories"], $id));
...
//                        $wpdb->query("UPDATE ".$wpdb->prefix."huge_itportfolio_portfolios SET  ht_show_filtering = '".$_POST["ht_show_filtering"]."'  WHERE id = '".$id."' ");
	...
	if(isset($_POST['changedvalues']) && $_POST['changedvalues'] != '') {
			...
			foreach ($rowim as $key=>$rowimages){
				...
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itportfolio_images SET  link_target = '%d'  WHERE ID = %d ", $_POST["sl_link_target".$rowimages->id.""], $rowimages->id));
